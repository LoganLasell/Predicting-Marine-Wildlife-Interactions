    ...
    mainPanel(
      tabsetPanel(
        tabPanel(
          ...
          withSpinner(imageOutput("posterior_plot"))
        )
      )
    )
  )
...
server <- function(input, output, session) {
  ...
  output$species_group_plot <- renderPlot({
    marine_catch %>%
      mutate(species_group = as.factor(species_group)) %>%   # ensure factor
      group_by(species_group) %>%
      summarise(
      high_value_individuals = sum(high_value_count, na.rm = TRUE),
      .groups = "drop"
      ) %>%
      ggplot(aes(
        x = fct_reorder(species_group, high_value_individuals),
      ...
      labs(
        x = "Species Group",
        y = "High-Value Individuals Caught",
        title = "High-Value Individuals by Species Group"
      ) +
      theme_minimal()
  })
  ...
  output$posterior_plot <- renderImage({
    list(
      src = "posterior_plot.png",
      contentType = "image/png",
      width = 800,
      height = 500,
      alt = "Posterior Predictive Probabilities Plot"
    )
  }, deleteFile = FALSE)  # keep the file, don’t delete
